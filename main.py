...
from vars import API_ID, API_HASH, BOT_TOKEN, OWNER, CREDIT, TOTAL_USERS
from db import db
...
# removed: 'from pyrogram.types.messages_and_media import message' — it shadowed handler params
...
async def add_auth_user(client: Client, message: Message):
    ...
    if len(args) < 1:
        await message.reply_text("**Usage:** `/addauth user_id [days]`\n_days defaults to 36500 (permanent)_")
        return
    try:
        new_user_id = int(args[0])
        days = int(args[1]) if len(args) > 1 else 36500
        try:
            user = await client.get_users(new_user_id)
            name = user.first_name + (f" {user.last_name}" if user.last_name else "")
        except Exception:
            name = f"User {new_user_id}"
        success, expiry = db.add_user(new_user_id, name, days)
        if success:
            expiry_str = expiry.strftime("%d-%m-%Y") if expiry else "Permanent"
            ...
            try:
                await client.send_message(new_user_id, f"🎉 **Access Granted!**\n\nYour subscription expires on **{expiry_str}**.")
            except Exception:
                pass
        else:
...
async def list_auth_users(client: Client, message: Message):
    ...
    for u in users:
        ...
        lines.append(f"• {u.get('name','?')} | `{u['user_id']}` | Exp: {expiry_str}")
    await message.reply_text("\n".join(lines))
...
async def remove_auth_user(client: Client, message: Message):
    ...
    try:
        ...
        if db.remove_user(user_id_to_remove):
            await message.reply_text(f"✅ **User `{user_id_to_remove}` removed.**")
            try:
                await client.send_message(user_id_to_remove, "⚠️ Your access has been revoked. Contact the owner to renew.")
            except Exception:
                pass
        else:
            await message.reply_text("❌ User not found in authorized list.")
    except (IndexError, ValueError):
        await message.reply_text("**Please provide a valid user ID.**")


# ── /plan — check your own subscription ──────────────────────────────────────
@bot.on_message(filters.command("plan") & filters.private)
async def my_plan_cmd(client: Client, message: Message):
    info = db.get_user_expiry_info(message.chat.id)
    if db.is_owner(message.chat.id):
        await message.reply_text("👑 You are the **Owner** — permanent access.")
        return
    if not info:
        await message.reply_text("❌ No active plan found.\n\nContact the owner to get access.")
        return
...
# ── /addchat — authorize a channel or group ───────────────────────────────────
@bot.on_message(filters.command("addchat") & filters.private)
async def add_chat_cmd(client: Client, message: Message):
    ...
    if len(args) < 1:
        await message.reply_text(
            "**Usage:** `/addchat chat_id [days]`\n"
            "_days defaults to 36500 (permanent)_\n\n"
            "Works for channels and groups (including forum/topic groups)."
        )
        return
    try:
        chat_id = int(args[0])
        ...
        try:
            ...
            chat_type = chat_obj.type.value
        except Exception:
            title     = str(chat_id)
            chat_type = "unknown"
        if db.add_chat(chat_id, title, chat_type, days):
            expiry_str = "Permanent" if days >= 36500 else f"{days} days"
            await message.reply_text(
                f"✅ **Chat Authorized**\n\n"
                f"📢 Title: {title}\n"
                f"🆔 ID: `{chat_id}`\n"
...
# ── /rmchat — deauthorize a channel or group ─────────────────────────────────
@bot.on_message(filters.command("rmchat") & filters.private)
async def rm_chat_cmd(client: Client, message: Message):
    if message.chat.id != OWNER:
        return
    try:
        ...
        if db.remove_chat(chat_id):
            ...
            await message.reply_text("❌ Chat not found in authorized list.")
    except (IndexError, ValueError):
        await message.reply_text("❌ Invalid chat ID.")


# ── /chats — list authorized channels/groups ──────────────────────────────────
@bot.on_message(filters.command("chats") & filters.private)
async def list_chats_cmd(client: Client, message: Message):
    if message.chat.id != OWNER:
        return
    chats = db.list_chats()
    if not chats:
        await message.reply_text("📝 No authorized chats found.")
        return
    lines = ["**📋 Authorized Chats**\n"]
    for c in chats:
        expiry = c.get("expiry_date")
        expiry_str = expiry.strftime("%d-%m-%Y") if hasattr(expiry, "strftime") else (str(expiry) if expiry else "Permanent")
        topics = c.get("allowed_topics", [])
        topic_str = f" | Topics: {topics}" if topics else ""
        lines.append(f"• {c.get('title','?')} | `{c['chat_id']}` | {c.get('chat_type','?')} | Exp: {expiry_str}{topic_str}")
    await message.reply_text("\n".join(lines))


# ── /addtopic — whitelist a topic thread inside a group ──────────────────────
@bot.on_message(filters.command("addtopic") & filters.private)
async def add_topic_cmd(client: Client, message: Message):
    """
    Usage: /addtopic group_chat_id topic_thread_id
    The group must already be authorized with /addchat.
    topic_thread_id is the message_thread_id shown when you right-click a topic.
    """
    if message.chat.id != OWNER:
        return
    args = message.command[1:]
    if len(args) < 2:
        await message.reply_text(
            "**Usage:** `/addtopic group_id topic_thread_id`\n\n"
            "First authorize the group with /addchat, then add topics.\n"
            "Get topic_thread_id by forwarding a message from the topic — it shows as message_thread_id."
        )
        return
    try:
        chat_id  = int(args[0])
        ...
        if not db.get_chat(chat_id):
            await message.reply_text("❌ Chat not authorized yet. Use /addchat first.")
            return
        if db.add_topic(chat_id, topic_id):
            await message.reply_text(f"✅ Topic `{topic_id}` added to chat `{chat_id}`.")
        else:
            await message.reply_text("❌ Failed to add topic.")
    except (IndexError, ValueError):
        await message.reply_text("❌ Invalid chat ID or topic ID.")


# ── /rmtopic — remove a topic from whitelist ─────────────────────────────────
@bot.on_message(filters.command("rmtopic") & filters.private)
async def rm_topic_cmd(client: Client, message: Message):
    if message.chat.id != OWNER:
        return
    args = message.command[1:]
    if len(args) < 2:
        await message.reply_text("**Usage:** `/rmtopic group_id topic_thread_id`")
        return
    try:
        chat_id  = int(args[0])
        topic_id = int(args[1])
        if db.remove_topic(chat_id, topic_id):
            await message.reply_text(f"✅ Topic `{topic_id}` removed from chat `{chat_id}`.")
        else:
            await message.reply_text("❌ Failed to remove topic.")
    except (IndexError, ValueError):
        await message.reply_text("❌ Invalid chat ID or topic ID.")
...
@bot.on_message(filters.command(["t2t"]) & (filters.private | filters.group | filters.channel))
async def text_to_txt(client, message: Message):
    ...
    if chat_type in ("channel", "group", "supergroup"):
        ...
        if not db.is_channel_authorized(message.chat.id, topic_id=topic_id):
            await message.reply_text("⚠️ This chat is not authorized. Contact the owner.")
            return
    else:
        _uid = message.from_user.id if message.from_user else 0
        if not db.is_user_authorized(_uid):
            await message.reply_text(f"<blockquote>**❌ You are not a Premium member.**\nYour ID: `{_uid}`</blockquote>")
            return
...
@bot.on_message(filters.command(["y2t"]) & (filters.private | filters.group | filters.channel))
...
async def cancel_handler(client: Client, m: Message):
    ...
    _uid = m.from_user.id if m.from_user else 0
    if not db.is_user_authorized(_uid):
        print(f"Unauthorized access attempt: {_uid}")
        await bot.send_message(
            m.chat.id,
            f"<blockquote>__**Oopss! You are not a Premium member**__\n"
            f"__**PLEASE /upgrade YOUR PLAN**__\n"
            f"__**Send me your user id for authorization**__\n"
            f"__**Your User id** __- `{_uid}`</blockquote>\n\n"
        )
    else:
...
async def start(bot, m: Message):
    ...
    # from_user is None for anonymous channel posts — fall back gracefully
    sender = m.from_user
    ...
    await asyncio.sleep(1)
    if db.is_user_authorized(m.chat.id):
        keyboard = InlineKeyboardMarkup([
...
async def txt_handler(bot: Client, m: Message):  
    ...
    processing_request = True
    cancel_requested = False
    # ── Auth check: private, channel, group, or group-topic ──────────────────
    chat_type = m.chat.type.value if hasattr(m.chat.type, "value") else str(m.chat.type)
    if chat_type in ("channel", "group", "supergroup"):
        topic_id = getattr(m, "message_thread_id", None)
        if not db.is_channel_authorized(m.chat.id, topic_id=topic_id):
            await m.reply_text("⚠️ This chat is not authorized. Contact the owner.")
            return
        user_id = m.from_user.id if m.from_user else 0
    else:
        user_id = m.from_user.id
        if not db.is_user_authorized(user_id):
            print(f"Unauthorized access attempt: {user_id}")
            await bot.send_message(
                m.chat.id,
    ...
    if "/d" in raw_text7:
        ...
        try:
            ...
            await m.reply_text("❌ Invalid channel ID. It must be a number like -100XXXXXXXXXX")
            return    
    await editable.delete()

    # ── Optional: topic thread ID for forum groups ────────────────────────────
    # editable was deleted above — use a fresh message
    topic_prompt = await m.reply_text(
        "**Send Topic Thread ID to upload into a group topic, or send /d to skip**\n\n"
        "<blockquote>Get it by using /id inside the topic, or right-click a topic message → Copy Link</blockquote>"
    )
    try:
        topic_input: Message = await bot.listen(topic_prompt.chat.id, filters=filters.user(m.from_user.id), timeout=20)
        raw_topic = topic_input.text.strip()
        await topic_input.delete(True)
    except asyncio.TimeoutError:
        raw_topic = "/d"
    await topic_prompt.delete()

    if raw_topic == "/d" or not raw_topic.lstrip("-").isdigit():
        upload_thread_id = None
    else:
        upload_thread_id = int(raw_topic)
    # ─────────────────────────────────────────────────────────────────────────

    try:
        if raw_text == "1":
            batch_message = await bot.send_message(chat_id=channel_id, text=f"<blockquote><b>🎯Target Batch : {b_name}</b></blockquote>", message_thread_id=upload_thread_id)
            ...
            try:
                ...
                if "drive" in url:
                    try:
                        ...
                        copy = await bot.send_document(chat_id=channel_id, message_thread_id=upload_thread_id,document=ka, caption=cc1)
                        count+=1
                        os.remove(ka)
                        ...
                        for attempt in range(max_retries):
                            try:
                                ...
                                if response.status_code == 200:
                                    ...
                                    copy = await bot.send_document(chat_id=channel_id, message_thread_id=upload_thread_id, document=f'{name}.pdf', caption=cc1)
                                    count += 1
                    ...
                    try:
                        ...
                        time.sleep(1)
                        await bot.send_document(chat_id=channel_id, message_thread_id=upload_thread_id, document=f"{name}.html", caption=cchtml)
                    ...
                    try:
                        ...
                        download_cmd = f"{cmd} -R 25 --fragment-retries 25"
                        os.system(download_cmd)
                        copy = await bot.send_photo(chat_id=channel_id, message_thread_id=upload_thread_id, photo=f'{name}.{ext}', caption=ccimg)
                        count += 1
                    ...
                    try:
                        ...
                        download_cmd = f"{cmd} -R 25 --fragment-retries 25"
                        os.system(download_cmd)
                        copy = await bot.send_document(chat_id=channel_id, message_thread_id=upload_thread_id, document=f'{name}.{ext}', caption=ccm)
                        count += 1
                        os.remove(f'{name}.{ext}')
                    except FloodWait as e:
                        await m.reply_text(str(e))
                    ...
                    prog = await bot.send_message(channel_id, Show, message_thread_id=upload_thread_id, disable_web_page_preview=True)
                    prog1 = await m.reply_text(Show1, disable_web_page_preview=True)
                    res_file = await helper.download_and_decrypt_video(url, cmd, name, appxkey)  
                    filename = res_file  
                    await prog1.delete(True)
                    await prog.delete(True)
                    await helper.send_vid(bot, m, cc, filename, thumb, name, prog, channel_id, message_thread_id=upload_thread_id)
                    count += 1  
                    await asyncio.sleep(1)  
                    continue  
...
async def text_handler(bot: Client, m: Message):
    if m.from_user.is_bot:
        return
    user_id = m.from_user.id
    # Auth check — same as /drm
    if not db.is_user_authorized(user_id):
        await m.reply_text(
            f"<blockquote>__**Oopss! You are not a Premium member\nPLEASE /upgrade YOUR PLAN\n"
            f"Your User id**__ - `{user_id}`</blockquote>"
        )
        return
    links = m.text
    path = None
...
def reset_and_set_commands():
    ...
    commands = [
        ...
        {"command": "addauth",  "description": "▶️ Add User (addauth id [days])"},
        {"command": "rmauth",   "description": "⏸️ Remove User"},
        {"command": "users",    "description": "👨‍👨‍👧‍👦 All Premium Users"},
        {"command": "plan",     "description": "📅 My Subscription Plan"},
        {"command": "addchat",  "description": "📢 Authorize Channel/Group"},
        {"command": "rmchat",   "description": "🚫 Deauthorize Channel/Group"},
        {"command": "chats",    "description": "📋 List Authorized Chats"},
        {"command": "addtopic", "description": "🧵 Whitelist Group Topic"},
        {"command": "rmtopic",  "description": "❌ Remove Group Topic"}
    ]
